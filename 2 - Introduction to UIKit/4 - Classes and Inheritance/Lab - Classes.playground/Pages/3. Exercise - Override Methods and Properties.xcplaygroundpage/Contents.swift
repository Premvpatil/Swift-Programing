...
class Fighter: Spaceship {
    ...
    override func moveLeft() {
        super.moveLeft()
        print("\(name) has moved left!")
    }

    override func moveRight() {
        super.moveRight()
        print("\(name) has moved right!")
    }
    ...
    override func wasHit() {
        super.wasHit()
        print("\(name) is damaged and lost health!")
    }
}
print("Welcome")
...
class ShieldedShip: Fighter {
    ...
    override func wasHit() {
        if shieldStrength > 0 {
            shieldStrength -= 5
            print("\(name) shield was hit! Shield strength: \(shieldStrength)")
        } else {
            health -= 5
            print("\(name) shield is down. Health is now: \(health)")
        }
    }
}
...
defender.fire()
print("Defender's remaining firepower: \(defender.remainingFirePower)")
